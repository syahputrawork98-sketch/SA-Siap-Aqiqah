...
import { Search, Filter, Plus, Edit, Trash2, Eye, Box, Package, RefreshCw, AlertCircle, Server } from "lucide-react";
...
export default function DataPaket() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const params = {
        search,
        jenisHewan: filterHewan === "Semua" ? undefined : filterHewan
      };
      const response = await dataMasterApi.getPackages(params);
  ...
  const stats = [
    { title: "Total Paket", value: meta?.total || data.length, icon: Box, color: "bg-blue-50 text-blue-600" },
    ...
    { title: "Rentang Harga", value: data.length > 0 ? `${(Math.min(...data.map(p => p.harga)) / 1000000).toFixed(1)}jt - ${(Math.max(...data.map(p => p.harga)) / 1000000).toFixed(1)}jt` : "0jt", icon: Package, color: "bg-purple-50 text-purple-600" },
  ];
      ...
      {error ? (
        <Card className="border-red-200 bg-red-50">
          <CardContent className="p-12 flex flex-col items-center justify-center text-center space-y-4">
            ...
            <div>
              <h3 className="text-lg font-bold text-red-800">Gagal Memuat Data Paket</h3>
          ...
          {loading && (
            <div className="absolute inset-0 bg-white/60 backdrop-blur-[1px] z-10 flex items-center justify-center">
              <div className="flex flex-col items-center gap-2">
                ...
                <span className="text-xs font-bold text-[#7a7368] uppercase tracking-widest">Sinkronisasi Paket...</span>
              </div>
            </div>
          )}
                  ...
                  {data.length > 0 ? (
                    ...
                    <tr>
                      <td colSpan="7" className="px-6 py-12 text-center">
                        <EmptyState message={loading ? "Memuat data paket..." : "Tidak ada data paket yang ditemukan."} />
                      </td>
                    </tr>
                  )}
                </tbody>
