...
import { Search, Filter, Plus, Edit, Trash2, Eye, Grid, Utensils } from "lucide-react";
import { formatCurrencyIdr } from "@/shared/lib";
import { MOCK_MENU } from "../../model/adminDataMasterData";

export default function DataMenu() {
  const [search, setSearch] = useState("");
  const [filterKategori, setFilterKategori] = useState("Semua");
  const [data] = useState(MOCK_MENU);
  ...
  const stats = [
    { title: "Total Menu", value: data.length, icon: Grid, color: "bg-blue-50 text-blue-600" },
                ...
                {filteredData.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan="7" className="px-6 py-12 text-center">
                      <EmptyState message="Tidak ada data menu yang ditemukan." />
                    </td>
                  </tr>
                )}
              </tbody>
