      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-2 space-y-6">
            ...
            {/* Timeline 2: Progres Produksi (Read-Only for Catering) */}
            <PublicCard className="p-8 space-y-6 bg-white border-none shadow-xl rounded-3xl">
              <div className="flex items-center gap-4 pb-4 border-b border-gray-100">
                <div className="w-12 h-12 bg-orange-50 text-orange-600 rounded-xl flex items-center justify-center">
                  <FiClock size={24} />
                  ...
                  <div className="relative space-y-6 before:absolute before:inset-0 before:ml-4 before:-translate-x-px before:h-full before:w-0.5 before:bg-gray-100">
                    {order.progress.map((step, idx) => (
                      <div key={idx} className="relative flex items-start gap-4">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 z-10 ${step.status === 'DONE' ? 'bg-orange-500 text-white' : 'bg-gray-100 text-gray-400'}`}>
            ...
            <PublicCard className="p-8 space-y-6 bg-white border-none shadow-xl rounded-3xl">
              ...
              <div className="pt-4 space-y-4">
                {myConfirmation?.status === 'PENDING' && ['PENDING_CONFIRMATION', 'Menunggu Konfirmasi'].includes(order.status) ? (
                  ...
                  <div className={`p-6 rounded-2xl text-center space-y-3 ${myConfirmation?.status === 'ACCEPTED' ? 'bg-orange-50 text-orange-700' : myConfirmation?.status === 'REJECTED' ? 'bg-red-50 text-red-700' : 'bg-gray-50 text-gray-400'}`}>
                    ...
                    <div className="space-y-1">
                      <p className="font-bold uppercase tracking-widest text-xs">
                         {myConfirmation?.status === 'ACCEPTED' ? 'Konfirmasi Diterima' : 
                          myConfirmation?.status === 'REJECTED' ? 'Konfirmasi Ditolak' : 
                          'Tugas Terkunci'}
                      </p>
